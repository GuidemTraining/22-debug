$(document).ready(function() {
    ...
    var banEndTime = 0;
    var completedTasks = 0; // Declare these variables in a higher scope
    var totalTasks = $('.guidem-button').length;
    // Wait for the DOM to be fully loaded
    
    $(window).on('load', function() {
        totalTasks = $('.guidem-button').length; // Get the count of .guidem-button elements
        ...
        function updateProgressBar(completed) {
            ...
            if (progressText) {
                progressText.text(`Your Progress: ${completed}/${totalTasks}`);
            }
        }
    ...
    function updateProgressBar(completed, total) {
        ...
        const progressText = $('#progress-text');
        if (progressText) {
            progressText.text(`Your Progress: ${completed}/${total}`);
        }
    }
